...
type Controller struct {
	// kubeclientset is a standard kubenetes clientset
	kubeclientset kubernetes.Interface
	// studentclientset is a clientset for our API group
...
func NewController(
	...
	eventBroadcaster.StartRecordingToSink(&typecorev1.EventSinkImpl{Interface: kubeclientset.CoreV1().Events("")})
	...
	studentInformer.Informer().AddEventHandler(cache.ReasourceEventHandlerFuncs{
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	for i := 0; i < threadiness; i++ {
		go wait.Until(c.runWorker, timeSecond, stopCh)
	}
...
func (c *Controller) syncHandler(key string) error {
	...
	if err != nil {
		runtime.HandlerError(fmt.Errorf("invalid resource key: %s", key))
		return nil
	}
	...
	c.recorder.Event(student, corev1.EventTypedNormal, SuccessSynced, MessagerourceSynced)
	return nil
}
